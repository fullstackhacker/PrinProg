...
static int variable()
{
	int reg; //register where the variable is stored

	if(!is_identifier(token)){ //token is not a letter
...
static int expr()
{
	...
	case '-': 
		next_token(); 
		left_reg = expr(); //generate code for storing the left part of the expression in a regsiter
		right_reg = expr(); //generate code for storing the right part of the expression in a register
		reg = next_register(); //get the next availible register
...
static void assign()
{	
	//current token is the variable letter
...
static void swap()
{
	...
	switch(token){
		case '%': 
			...
			CodeGen(LOAD, reg1, a, EMPTY_FIELD);
			CodeGen(LOAD, reg2, b, EMPTY_FIELD); 
			CodeGen(STORE, a, reg2, EMPTY_FIELD); 
			CodeGen(STORE, b, reg1, EMPTY_FIELD);
			next_token();
			break; 
		default:
			ERROR("Symbol %c unknown\n", token);
